...
limit 1
    ...
    WHERE NOT EXISTS (SELECT 1 FROM __dbt__cte__my_next_challenge) -- noqa: PRS
    RETURNING challenge_id
)

SELECT
    coalesce(
        my_next_challenge.challenge_id, created.challenge_id
    ) AS challenge_id
FROM __dbt__cte__my_next_challenge AS my_next_challenge
FULL OUTER JOIN created ON true