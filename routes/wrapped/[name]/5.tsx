...
export default async function MyPage(
  ...
  const myName = name;
  const [my_errors, errors, images] = await Promise.all([
    runSql({
      args: { name },
      file: "wrapped/top_errors",
      connection: ctx.state.connection,
    }),
    runSql({
      file: "wrapped/top_errors_overall",
      connection: ctx.state.connection,
    }),
    runSql({
      args: { name },
      file: "wrapped/wordcloud_images",
      connection: ctx.state.connection,
    }),
  ]);
  return (
    ...
    >
      ...
      <h2>Next up...</h2>
      <div>
        The top mispelled word of {name} was{"  "}
      ...
      />
      <div>
        The overall most popular mispelled word was{"  "}
        <span class="font-bold">
          {errors[0].errors}
        </span>
        , which was used{" "}
